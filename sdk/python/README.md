...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `pczora` with the `ORG` of your provider.
   - replace all instances of the `github.com/pczora/pulumi-gandi` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-gandi
  ...
  ignore: []
  ldflags:
  - -X github.com/pczora/pulumi-gandi/provider/pkg/version.Version={{.Tag}}
