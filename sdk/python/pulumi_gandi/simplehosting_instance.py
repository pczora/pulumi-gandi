# coding=utf-8
# *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import copy
import warnings
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
from . import _utilities

__all__ = ['SimplehostingInstanceArgs', 'SimplehostingInstance']

@pulumi.input_type
class SimplehostingInstanceArgs:
    def __init__(__self__, *,
                 database_name: pulumi.Input[str],
                 language_name: pulumi.Input[str],
                 location: pulumi.Input[str],
                 size: pulumi.Input[str],
                 name: Optional[pulumi.Input[str]] = None):
        """
        The set of arguments for constructing a SimplehostingInstance resource.
        :param pulumi.Input[str] database_name: The name of the database type ('mysql' or 'pgsql')
        :param pulumi.Input[str] language_name: The name of the language ('php', 'python', 'nodejs' or 'ruby')
        :param pulumi.Input[str] location: The datacenter location of the instance ('FR' or 'LU')
        :param pulumi.Input[str] size: The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        :param pulumi.Input[str] name: The name of the SimpleHosting instance
        """
        pulumi.set(__self__, "database_name", database_name)
        pulumi.set(__self__, "language_name", language_name)
        pulumi.set(__self__, "location", location)
        pulumi.set(__self__, "size", size)
        if name is not None:
            pulumi.set(__self__, "name", name)

    @property
    @pulumi.getter(name="databaseName")
    def database_name(self) -> pulumi.Input[str]:
        """
        The name of the database type ('mysql' or 'pgsql')
        """
        return pulumi.get(self, "database_name")

    @database_name.setter
    def database_name(self, value: pulumi.Input[str]):
        pulumi.set(self, "database_name", value)

    @property
    @pulumi.getter(name="languageName")
    def language_name(self) -> pulumi.Input[str]:
        """
        The name of the language ('php', 'python', 'nodejs' or 'ruby')
        """
        return pulumi.get(self, "language_name")

    @language_name.setter
    def language_name(self, value: pulumi.Input[str]):
        pulumi.set(self, "language_name", value)

    @property
    @pulumi.getter
    def location(self) -> pulumi.Input[str]:
        """
        The datacenter location of the instance ('FR' or 'LU')
        """
        return pulumi.get(self, "location")

    @location.setter
    def location(self, value: pulumi.Input[str]):
        pulumi.set(self, "location", value)

    @property
    @pulumi.getter
    def size(self) -> pulumi.Input[str]:
        """
        The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        return pulumi.get(self, "size")

    @size.setter
    def size(self, value: pulumi.Input[str]):
        pulumi.set(self, "size", value)

    @property
    @pulumi.getter
    def name(self) -> Optional[pulumi.Input[str]]:
        """
        The name of the SimpleHosting instance
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "name", value)


@pulumi.input_type
class _SimplehostingInstanceState:
    def __init__(__self__, *,
                 database_name: Optional[pulumi.Input[str]] = None,
                 language_name: Optional[pulumi.Input[str]] = None,
                 location: Optional[pulumi.Input[str]] = None,
                 name: Optional[pulumi.Input[str]] = None,
                 size: Optional[pulumi.Input[str]] = None):
        """
        Input properties used for looking up and filtering SimplehostingInstance resources.
        :param pulumi.Input[str] database_name: The name of the database type ('mysql' or 'pgsql')
        :param pulumi.Input[str] language_name: The name of the language ('php', 'python', 'nodejs' or 'ruby')
        :param pulumi.Input[str] location: The datacenter location of the instance ('FR' or 'LU')
        :param pulumi.Input[str] name: The name of the SimpleHosting instance
        :param pulumi.Input[str] size: The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        if database_name is not None:
            pulumi.set(__self__, "database_name", database_name)
        if language_name is not None:
            pulumi.set(__self__, "language_name", language_name)
        if location is not None:
            pulumi.set(__self__, "location", location)
        if name is not None:
            pulumi.set(__self__, "name", name)
        if size is not None:
            pulumi.set(__self__, "size", size)

    @property
    @pulumi.getter(name="databaseName")
    def database_name(self) -> Optional[pulumi.Input[str]]:
        """
        The name of the database type ('mysql' or 'pgsql')
        """
        return pulumi.get(self, "database_name")

    @database_name.setter
    def database_name(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "database_name", value)

    @property
    @pulumi.getter(name="languageName")
    def language_name(self) -> Optional[pulumi.Input[str]]:
        """
        The name of the language ('php', 'python', 'nodejs' or 'ruby')
        """
        return pulumi.get(self, "language_name")

    @language_name.setter
    def language_name(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "language_name", value)

    @property
    @pulumi.getter
    def location(self) -> Optional[pulumi.Input[str]]:
        """
        The datacenter location of the instance ('FR' or 'LU')
        """
        return pulumi.get(self, "location")

    @location.setter
    def location(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "location", value)

    @property
    @pulumi.getter
    def name(self) -> Optional[pulumi.Input[str]]:
        """
        The name of the SimpleHosting instance
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "name", value)

    @property
    @pulumi.getter
    def size(self) -> Optional[pulumi.Input[str]]:
        """
        The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        return pulumi.get(self, "size")

    @size.setter
    def size(self, value: Optional[pulumi.Input[str]]):
        pulumi.set(self, "size", value)


warnings.warn("""gandi.index/simplehostinginstance.SimplehostingInstance has been deprecated in favor of gandi.simplehosting/instance.Instance""", DeprecationWarning)


class SimplehostingInstance(pulumi.CustomResource):
    warnings.warn("""gandi.index/simplehostinginstance.SimplehostingInstance has been deprecated in favor of gandi.simplehosting/instance.Instance""", DeprecationWarning)

    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 database_name: Optional[pulumi.Input[str]] = None,
                 language_name: Optional[pulumi.Input[str]] = None,
                 location: Optional[pulumi.Input[str]] = None,
                 name: Optional[pulumi.Input[str]] = None,
                 size: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        """
        Create a SimplehostingInstance resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] database_name: The name of the database type ('mysql' or 'pgsql')
        :param pulumi.Input[str] language_name: The name of the language ('php', 'python', 'nodejs' or 'ruby')
        :param pulumi.Input[str] location: The datacenter location of the instance ('FR' or 'LU')
        :param pulumi.Input[str] name: The name of the SimpleHosting instance
        :param pulumi.Input[str] size: The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: SimplehostingInstanceArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Create a SimplehostingInstance resource with the given unique name, props, and options.
        :param str resource_name: The name of the resource.
        :param SimplehostingInstanceArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(SimplehostingInstanceArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 database_name: Optional[pulumi.Input[str]] = None,
                 language_name: Optional[pulumi.Input[str]] = None,
                 location: Optional[pulumi.Input[str]] = None,
                 name: Optional[pulumi.Input[str]] = None,
                 size: Optional[pulumi.Input[str]] = None,
                 __props__=None):
        pulumi.log.warn("""SimplehostingInstance is deprecated: gandi.index/simplehostinginstance.SimplehostingInstance has been deprecated in favor of gandi.simplehosting/instance.Instance""")
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = SimplehostingInstanceArgs.__new__(SimplehostingInstanceArgs)

            if database_name is None and not opts.urn:
                raise TypeError("Missing required property 'database_name'")
            __props__.__dict__["database_name"] = database_name
            if language_name is None and not opts.urn:
                raise TypeError("Missing required property 'language_name'")
            __props__.__dict__["language_name"] = language_name
            if location is None and not opts.urn:
                raise TypeError("Missing required property 'location'")
            __props__.__dict__["location"] = location
            __props__.__dict__["name"] = name
            if size is None and not opts.urn:
                raise TypeError("Missing required property 'size'")
            __props__.__dict__["size"] = size
        super(SimplehostingInstance, __self__).__init__(
            'gandi:index/simplehostingInstance:SimplehostingInstance',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None,
            database_name: Optional[pulumi.Input[str]] = None,
            language_name: Optional[pulumi.Input[str]] = None,
            location: Optional[pulumi.Input[str]] = None,
            name: Optional[pulumi.Input[str]] = None,
            size: Optional[pulumi.Input[str]] = None) -> 'SimplehostingInstance':
        """
        Get an existing SimplehostingInstance resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[str] database_name: The name of the database type ('mysql' or 'pgsql')
        :param pulumi.Input[str] language_name: The name of the language ('php', 'python', 'nodejs' or 'ruby')
        :param pulumi.Input[str] location: The datacenter location of the instance ('FR' or 'LU')
        :param pulumi.Input[str] name: The name of the SimpleHosting instance
        :param pulumi.Input[str] size: The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = _SimplehostingInstanceState.__new__(_SimplehostingInstanceState)

        __props__.__dict__["database_name"] = database_name
        __props__.__dict__["language_name"] = language_name
        __props__.__dict__["location"] = location
        __props__.__dict__["name"] = name
        __props__.__dict__["size"] = size
        return SimplehostingInstance(resource_name, opts=opts, __props__=__props__)

    @property
    @pulumi.getter(name="databaseName")
    def database_name(self) -> pulumi.Output[str]:
        """
        The name of the database type ('mysql' or 'pgsql')
        """
        return pulumi.get(self, "database_name")

    @property
    @pulumi.getter(name="languageName")
    def language_name(self) -> pulumi.Output[str]:
        """
        The name of the language ('php', 'python', 'nodejs' or 'ruby')
        """
        return pulumi.get(self, "language_name")

    @property
    @pulumi.getter
    def location(self) -> pulumi.Output[str]:
        """
        The datacenter location of the instance ('FR' or 'LU')
        """
        return pulumi.get(self, "location")

    @property
    @pulumi.getter
    def name(self) -> pulumi.Output[str]:
        """
        The name of the SimpleHosting instance
        """
        return pulumi.get(self, "name")

    @property
    @pulumi.getter
    def size(self) -> pulumi.Output[str]:
        """
        The size of the SimpleHosting instance ('s+', 'm', 'l' or 'xxl')
        """
        return pulumi.get(self, "size")

